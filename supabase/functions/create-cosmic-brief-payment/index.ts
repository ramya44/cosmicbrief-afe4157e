...
serve(async (req) => {
  ...
  try {
    ...
    // Note: success_url includes type=cosmic-brief to route to correct generation function
    const session = await stripe.checkout.sessions.create({
      customer: customerId,
      customer_email: customerId ? undefined : email,
      line_items: [
        {
          price: priceIds.cosmicBrief,
          quantity: 1,
        },
      ],
      mode: "payment",
      allow_promotion_codes: true,
      success_url: `${appUrl}/#/vedic/payment-success?session_id={CHECKOUT_SESSION_ID}&kundli_id=${kundli_id}&type=cosmic-brief`,
      cancel_url: `${appUrl}/#/vedic/results?id=${kundli_id}&type=cosmic-brief`,
      metadata: {
        kundli_id,
        product_type: "cosmic-brief",
