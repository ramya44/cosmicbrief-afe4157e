...
const logStep = createLogger("SAVE-KUNDLI-DETAILS");
...
const KundliInputSchema = z.object({
  ...
  kundli_data: z.object({
    ...
    dasha_periods: z.array(MahaDashaSchema).max(12).optional().default([]),
  }),
});
...
serve(async (req) => {
  ...
  try {
    ...
    if (existingKundliId) {
      ...
      logStep("Updating existing kundli");
      ...
      // Insert new kundli
      logStep("Inserting new kundli details into database");
    ...
    logStep("Kundli details saved successfully", { id: data.id, updated: !!existingKundliId });
