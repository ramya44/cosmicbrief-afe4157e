...
export function getCurrentMahaDasha(
  ...
  return dashas.find(d => currentDate >= d.start_date && currentDate <= d.end_date) || null;
}
...
export function getCurrentAntarDasha(
  ...
  const mahaDasha = getCurrentMahaDasha(birthDate, moonLongitude, currentDate);
  if (!mahaDasha) return null;
  
  const antarDashas = calculateAntarDashas(mahaDasha);
  return antarDashas.find(d => currentDate >= d.start_date && currentDate <= d.end_date) || null;
}

/**
 * Get all Dasha changes in a given year
 */
export function getDashaChangesInYear(
  birthDate: Date,
  moonLongitude: number,
  year: number
): { mahaDashaChanges: DashaPeriod[]; antarDashaChanges: AntarDashaPeriod[] } {
  ...
  // Find maha dashas that start in this year
  const mahaDashaChanges = allMahaDashas.filter(d => 
    d.start_date >= yearStart && d.start_date <= yearEnd
  );
  
  // Get antar dashas for maha dashas active during this year
  const activeMahaDashas = allMahaDashas.filter(d =>
    (d.start_date <= yearEnd && d.end_date >= yearStart)
  );
  
  const antarDashaChanges: AntarDashaPeriod[] = [];
  for (const mahaDasha of activeMahaDashas) {
    const antarDashas = calculateAntarDashas(mahaDasha);
    const yearAntarDashas = antarDashas.filter(d =>
      d.start_date >= yearStart && d.start_date <= yearEnd
    );
    antarDashaChanges.push(...yearAntarDashas);
  }
  
  return { mahaDashaChanges, antarDashaChanges };
}
 ...
 */
export function processPlanetaryPositions(
  ...
  dashaChanges2026: { mahaDashaChanges: DashaPeriod[]; antarDashaChanges: AntarDashaPeriod[] };
} {
  // Find Moon position
  ...
  // Get 2026 changes
  const dashaChanges2026 = getDashaChangesInYear(birthDate, moonLongitude, 2026);
  
  return {
    nakshatra: { name: nakshatra.name, lord: nakshatra.lord },
    currentMahaDasha,
    currentAntarDasha,
    dashaChanges2026
  };
...
export type { PlanetaryPosition, DashaPeriod, AntarDashaPeriod };
